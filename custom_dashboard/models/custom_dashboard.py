...
class CustomDashboard(models.Model):
    ...
    def get_custom_info(self):
        ...
        my_leave_request = 0
        request_sale_orders = 0
        request_contacts = 0
        request_invoices = 0
        request_borrowings = 0
        request_planning = 0
        request_documents = 0
        request_purchases = 0
        request_projects = 0
        ...
        employee_obj = self.env['hr.employee'].sudo().search([('user_id', '=', uid)], limit=1)
        employee = self.env['sale.order'].sudo().search_read([('user_id', '=', uid)], limit=1)

        if employee_obj:
            employee_id = employee_obj.id
            my_leave_request = self.env['hr.leave'].sudo().search_count([('employee_id','=',employee_obj.id),('state','=','confirm')])
            request_sale_orders = self.env['sale.order'].sudo().search_count([('state','=','waiting_for_approval')])
            request_contacts = self.env['res.partner'].sudo().search_count([('state','=','waiting_approval')])
            request_invoices = self.env['account.move'].sudo().search_count([('move_type', '=', 'out_invoice')])
            request_borrowings = self.env['borrow.borrow'].sudo().search_count([])
            request_planning = self.env['planning.slot'].sudo().search_count([('employee_id','=',employee_obj.id)])
            request_documents = self.env['document.document'].sudo().search_count([])
            request_purchases = self.env['purchase.order'].sudo().search_count([])
            request_projects = self.env['project.project'].sudo().search_count([])

        cardex_id.update({
            'uid': uid,
            'employee_id': employee_id,
            'employee': employee,
            'my_leave_request': my_leave_request,
            'my_leave_request_form_id': self.env.ref('hr_holidays.hr_leave_view_form').id,
            'request_sale_orders':request_sale_orders,
            'request_contacts':request_contacts,
            'request_invoices':request_invoices,
            'request_borrowings':request_borrowings,
